...
IF NOT EXISTS (SELECT 1 FROM sys.all_objects WHERE name = 'SP_INSERE_MUSICA' AND type = 'P')
BEGIN
	EXEC ('CREATE PROCEDURE SP_INSERE_MUSICA AS SELECT 1')
END
GO

ALTER PROCEDURE SP_INSERE_MUSICA (@CODIGO_MUSICA INT, @NOME_MUSICA VARCHAR(100), @DURACAO DECIMAL(3,2))
AS 

	INSERT INTO MUSICA VALUES (@CODIGO_MUSICA, @NOME_MUSICA, @DURACAO)

GO 
EXEC SP_INSERE_MUSICA 123, 'AMIANTO', 4.17
