...
ALTER PROCEDURE RETORNA_ULTIMO_REGISTRO (@TABELA VARCHAR(MAX), @QTD_REGISTROS INT, @CONT INT, @QTD_REGISTROS_SAIDA INT OUTPUT, @CONT_SAIDA INT OUTPUT)
	AS 
	BEGIN
...
ALTER PROCEDURE NUMERO_ALEATORIO(@MIN INT , @MAX INT, @RETORNO INT OUTPUT)
AS 
BEGIN 
	SELECT @RETORNO =  ROUND(RAND() * @MAX - @MIN, 0) + @MIN 
END
GO
...
ALTER PROCEDURE PREENCHE_DB_CD (@TABELA VARCHAR(MAX), @QTD_REGISTROS INT)
AS 
BEGIN
	...
	BEGIN
		...
		BEGIN
			DECLARE 
				...
				-- RETORNA O TEMPO DA MÚSICA ENTRE 60 E 1000 SEGUNDOS
				EXEC NUMERO_ALEATORIO 60, 1000, @RETORNO = @TEMPO_MUSICA  OUTPUT
	...
	PRINT @CONT
		...
		BEGIN
			DECLARE 
				...
				EXEC NUMERO_ALEATORIO 10, 50,  @RETORNO = @MENOR_PRECO OUTPUT 
				EXEC NUMERO_ALEATORIO 11, 100, @RETORNO = @MAIOR_PRECO OUTPUT 
